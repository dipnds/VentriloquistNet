...
from networks.gan_cnn_emo_4 import (Generator, Discriminator_RealFakeSeq,
...
def train(G, D_rf, prTr_emo_model, prTr_CE, epoch): # D_ls
    ...
    for batch, (mel, mfcc, target_kp, _) in tr_batch:
        ...
        lossG_lower = cr_lipDist(pred_kp,target_kp)
    ...
    if (epoch+1)%20 == 0:
        torch.save(G, modelpath+str(epoch+1)+comment+'Tr_G.model')
        torch.save(D_rf, modelpath+str(epoch+1)+comment+'Tr_D_rf.model')
    
    # torch.save(pred_kp,'kp/'+str(epoch)+'pred_kp_'+comment+'.pt')
