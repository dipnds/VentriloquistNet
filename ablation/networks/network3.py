...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.rnn1 = nn.LSTM(68*2+1024, 320, 1, batch_first=True, bidirectional=True)
        self.rnn2 = nn.LSTM(640, 160, 1, batch_first=True, bidirectional=True)
        self.rnn3 = nn.LSTM(320, 68*2, 1, batch_first=True, bidirectional=True)
    ...
    def forward(self, key0, mel, orienT):
        ...
        orienT = torch.unsqueeze(orienT, dim=0)
        orienT = orienT.repeat(2, 1, 1)
        _,(keyT,_) = self.rnn3(keyT,(orienT,torch.zeros_like(orienT)))
